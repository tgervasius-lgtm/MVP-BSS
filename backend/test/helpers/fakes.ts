...
export class FakePhaseAService implements MvpService {
  ...
  async listTerminalSyncEvents(_actor: ActorContext, terminalId: string, _filters: { from: string; to: string; eventStatus?: TerminalSyncEventView["status"]; cursor?: string; limit: number }): Promise<Page<TerminalSyncEventView>> { return { items: [{ id: IDS.card, terminalId, deviceEventId: IDS.worker, sequence: 1, workerId: IDS.worker, occurredAt: new Date(0).toISOString(), acknowledgedAt: new Date(1000).toISOString(), receivedAt: new Date(2000).toISOString(), eventType: "check_in", status: "synced", rejectionCode: null, attendanceEventId: IDS.attendance, acknowledgementVerified: true, lifecycleEvidence: { decision: "accepted", code: null, acknowledgement: { verified: true, delaySeconds: 1 } } }], page: { nextCursor: null, total: 1 } }; }
  ...
  async rotateTerminalCredential(_actor: ActorContext, _terminalId: string, _input: TerminalCredentialRotationWrite, _revision: string, _requestId: string): Promise<TerminalCredentialRotationView> { return { terminal: { ...this.terminal(), revision: "2" }, deviceCredential: "rotated-device-secret", acknowledgementKey: { id: IDS.request, version: 2, derivation: "BSS-TERMINAL-ACK-KEY-V1" } }; }
  ...
  async resolveTerminalEventReconciliation(_actor: ActorContext, attendanceEventId: string, input: TerminalEventReconciliationWrite, _requestId: string): Promise<TerminalEventReconciliationView> { return { id: IDS.request, attendanceEventId, resolution: input.resolution, reason: input.reason, attendanceDayId: input.resolution === "accepted" ? IDS.attendance : null, resolvedBy: IDS.user, createdAt: new Date(0).toISOString() }; }

(function installApiBindings(root){
  ...
  async function bootstrapApi(){
    try{
      await hydrateApi();
      installApiBindings();resetApiFilters();render();
    }catch(error){
      if(!Number.isInteger(error?.status)||error.status===404)return;
      installApiBindings();logged=false;state=createApiState();if(error.status!==401)apiError=apiMessage(error);render();
    }
  }

...
| Backend quality | TypeScript, build, unit/contract/integration tests and PostgreSQL flows pass | AUTOMATED | Backend quality gate plus full-stack quality gate; PR #99 passed integration acceptance; PR #174 merged isolated PostgreSQL fixtures and PR #176 merged the reproduced RFID ordering correction; focused RED-to-GREEN evidence is in `docs/security/RFID_ASSIGNMENT_ORDERING.md`; repository checks do not establish staging or production acceptance |
...
| Dependency maintenance | Dependency updates and new vulnerabilities/licences are continuously reviewed | PARTIAL / AUTOMATED | Root npm, `/backend` npm and GitHub Actions Dependabot version-update streams and full high-severity graph audits are active; R34 pins Fastify 5.12.5 and rate-limit 11.2.0 with evidence/limits in `docs/security/R34_HTTP_DEPENDENCIES.md`; B3 static-file security/Node compatibility is tracked separately in PR #167 / issue #177 and `docs/security/B3_STATIC_FILES.md`, with native Windows and minimum-Node CI evidence required; repository-level Dependabot alerts/security updates remain issue #115 |

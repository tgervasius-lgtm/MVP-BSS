...
if(!BSS_CORE?.contracts || !BSS_CORE?.time || !BSS_CORE?.access || !BSS_CORE?.runtime || !BSS_CORE?.api || !BSS_CORE?.apiState ||
...
const BSS_API = BSS_CORE.api;
const BSS_API_STATE = BSS_CORE.apiState;
...
const DEFAULT_STATE = {
  version: 8,
  demoMode: true,
  sharedLeaveVisibility: 'department',
  company: {
    name: 'BSS Demo d.o.o.',
    oib: '12345678903',
...
let workerSearch = '';
let workerShiftFilter = 'Svi';
let calendarYear = Number(DEMO_TODAY.slice(0,4));
let calendarMonth = Number(DEMO_TODAY.slice(5,7))-1;
...
let reportFilters = {month: CURRENT_MONTH, department: 'Svi', workerId: 'Svi', type: 'summary'};
...
function createApiState(){
  const empty=clone(DEFAULT_STATE);
  Object.assign(empty,{
    demoMode:false,
    company:{name:'BSS',oib:'',address:'',timezone:'Europe/Zagreb',workTime:'',contactEmail:'',phone:'',defaultVacationAllowance:20,revision:'0'},
    departments:[],jobPositions:[],holidays:[],shifts:[],workers:[],accessUsers:[],invitations:[],records:[],requests:[],corrections:[],audit:[],
    terminal:{id:null,apiId:null,name:'Terminal',location:'',online:false,unsynced:0,scans:0,lastSync:'Nema podataka',lastHeartbeat:'Nema podataka',version:'',queue:[],recentEvents:[],syncRuns:[],revision:'0'},
    lastScan:null,lastReport:'Nije još generiran',reportHistory:[]
  });
  return empty;
}
function loadState(){ return createApiState(); }
function saveState(){ /* Poslovni podaci spremaju se isključivo preko API-ja. */ }
...
function resetState(){ state = createApiState(); }
...
function currentWorker(){ return workerById(role().userId) || (!sessionContext?state.workers[0]:null) || {id:0,name:sessionContext?.user?.email||'Korisnik',email:sessionContext?.user?.email||'',dept:'',jobTitle:'',shiftId:0,status:'Odsutna',card:'',cardStatus:'Nije dodijeljena',todayStart:'',active:true,vacationAllowance:0}; }
...
function dashboardMetrics(workers){
  ...
  const records=state.records.filter(record=>ids.has(record.workerId)&&record.date.startsWith(CURRENT_MONTH));
  const kpiValue=(id,fallback)=>Number(state.dashboard?.kpis?.find(item=>item.id===id)?.value??fallback);
  const derivedPresent=workers.filter(worker=>['Prisutan','Kasni'].includes(worker.status)).length;
  const derivedAbsent=workers.filter(worker=>['Odsutna','Godišnji','Bolovanje'].includes(worker.status)).length;
  const derivedReview=records.filter(record=>['Kašnjenje','Nepotpun zapis'].includes(record.status)).length;
  return {
    active:workers.length,
    present:kpiValue('present',derivedPresent),
    late:workers.filter(worker=>worker.status==='Kasni').length,
    absent:kpiValue('absent_today',derivedAbsent),
    vacation:0,
    sick:0,
    waiting:workers.filter(worker=>worker.status==='Očekuje smjenu').length,
    pending:kpiValue('pending_decision',pendingLeaveCount()),
    review:kpiValue('review_required',derivedReview),
...
function weeklyAttendance(workerIds){
  ...
  const dates=[...new Set(state.records.filter(record=>ids.has(record.workerId)).map(record=>record.date))].sort().slice(-5);
...
function switchRole(next){
  ...
  attendanceView='all';
  myTimeMonth='2026-07';
  myTimeReviewOnly=false;workerShiftFilter='Svi';
...
function bottomNavigation(){
  const picks = {
    ...
    accountant: ['home','reports','vacations','sharedLeave']
  }[currentRole] || ['home'];
...
function shell(){
  ...
  const authFields=invitationMode
    ? `<div class="notice info"><b>Aktivacija korisničkog računa</b><br>Postavi sigurnu lozinku za dovršetak pozivnice.</div><label>Nova lozinka<input id="loginPassword" type="password" autocomplete="new-password"></label>`
    : '<label>Email<input id="loginEmail" type="email" autocomplete="username"></label><label>Lozinka<input id="loginPassword" type="password" autocomplete="current-password"></label>';
...
function openAttendanceStatus(status){
  attendanceFilters={month:CURRENT_MONTH,department:'Svi',status,search:''};
  attendanceView='all';
  navigate('attendance');
}
function openAttendanceReview(){
  attendanceFilters={month:CURRENT_MONTH,department:'Svi',status:'Svi',search:''};
  attendanceView='review';
  navigate('attendance');
}
function openPendingRequests(){ openVacationRequestStatus('Na čekanju'); }
function openVacationRequestStatus(status){
  if(!['Svi',...REQUEST_STATUSES].includes(status))return;
  requestStatusFilter=status;requestSearch='';navigate('requests');
}
function openWorkerStatus(tab){ workerListTab=tab;workerSearch='';workerShiftFilter='Svi';navigate('workers'); }
...
function setWorkerTab(tab){ workerListTab=tab;workerShiftFilter='Svi';render(); }
...
function filteredWorkers(){
  ...
  return visibleWorkers().filter(worker=>{
    const tabMatch = workerListTab==='Svi' ? worker.active : workerListTab==='Prisutni' ? worker.active&&['Prisutan','Kasni'].includes(worker.status) : workerListTab==='Odsutni danas' ? worker.active&&['Odsutna','Godišnji','Bolovanje'].includes(worker.status) : workerListTab==='Godišnji' ? worker.active&&worker.status==='Godišnji' : !worker.active;
    const shiftMatch=workerShiftFilter==='Svi'||worker.shiftId===Number(workerShiftFilter);
...
function openShiftWorkers(id){
  const shift=shiftById(id);if(!shift)return;
  workerListTab='Svi';workerSearch='';workerShiftFilter=shift.id;navigate('workers');
}
...
function vacationRequestPreviewText(type,start,end,workerId){
  ...
  if(type!=='Godišnji odmor')return `${pluralDays(days)} · slobodan dan ne umanjuje fond godišnjeg odmora.`;
...
function submitVacationRequest(){
  ...
  const validation=BSS_USE_CASES.leave.validateSubmission({
    workerId:worker.id,type,start,end,today:DEMO_TODAY,year:DEMO_TODAY.slice(0,4),requests:state.requests,
  ...
  if(!validation.ok){
    const messages={
      ...
      OUTSIDE_YEAR:'Zahtjev mora biti unutar tekuće godine.',
...
function calendarMonthCard(year,month,requests,large=false,dayAction='showVacationDay',nameOnly=false){
  ...
  for(let day=1;day<=dayCount;day++){
    ...
    if(events.length){
      const names=events.map(event=>nameOnly?(workerById(event.workerId)?.name||'Radnik'):`${workerById(event.workerId)?.name||'Radnik'} – ${event.status}`).join(', ');
      cells+=`<button class="day ${statusClass} ${today} ${weekend}" data-bss-action="${dayAction}('${iso}')" aria-label="${escapeHtml(`${isoLabel(iso)}: ${names}`)}" title="${escapeHtml(names)}">${day}${events.length>1?`<span class="event-count">${events.length}</span>`:''}</button>`;
    }else{
...
function sharedLeaveTeamDepartments(){
  if(currentRole==='manager'&&role().departments?.length)return [...role().departments];
  const ownDepartment=departmentByName(currentWorker().dept);
  if(!ownDepartment?.managerId)return [currentWorker().dept];
  return (state.departments||[]).filter(item=>item.active&&item.managerId===ownDepartment.managerId).map(item=>item.name);
}
function sharedLeaveDepartments(){
  const visibility=sharedLeaveVisibility();
  if(visibility==='organization')return departmentList();
  if(visibility==='team')return sharedLeaveTeamDepartments();
  return [currentWorker().dept];
}
function sharedLeaveScopeLabel(){
  const visibility=sharedLeaveVisibility(),departments=sharedLeaveDepartments();
  if(visibility==='organization')return 'Cijela organizacija';
  if(visibility==='team')return `Tim · ${departments.join(', ')}`;
  return `Odjel · ${departments[0]||currentWorker().dept}`;
}
function sharedLeaveRequests(){
  const departments=new Set(sharedLeaveDepartments());
  return state.requests.filter(request=>{
    const worker=workerById(request.workerId);
    return request.type==='Godišnji odmor'&&request.status==='Odobreno'&&worker?.active&&departments.has(worker.dept);
  }).sort((a,b)=>a.start.localeCompare(b.start)||a.workerId-b.workerId);
}
function setSharedLeaveVisibility(value){
  if(currentRole!=='admin'||!SHARED_LEAVE_VISIBILITY.includes(value))return;
  state.sharedLeaveVisibility=value;
  audit('Administrator',`Promijenjena demo-vidljivost zajedničkog godišnjeg: ${value}.`,'Postavke');
  saveAndRender('Vidljivost zajedničkog godišnjeg je ažurirana u frontend demu.');
}
...
function sharedLeaveScopeControl(){
  const labels={team:'Tim',department:'Odjel',organization:'Organizacija'},active=sharedLeaveVisibility();
  if(currentRole!=='admin')return `<div class="shared-scope-readonly"><span>Vidljivost</span><b>${escapeHtml(sharedLeaveScopeLabel())}</b></div>`;
  return `<div class="shared-scope-control"><div><span>Vidljivost zaposlenicima</span><b>${escapeHtml(sharedLeaveScopeLabel())}</b></div><div class="scope-switch" role="group" aria-label="Vidljivost zajedničkog godišnjeg">${SHARED_LEAVE_VISIBILITY.map(value=>`<button class="${active===value?'active':''}" data-bss-action="setSharedLeaveVisibility('${value}')">${labels[value]}</button>`).join('')}</div></div>`;
}
...
function normalizeReportFilters(filters=reportFilters){
  const month=/^\d{4}-(0[1-9]|1[0-2])$/.test(filters?.month||'')?filters.month:CURRENT_MONTH;

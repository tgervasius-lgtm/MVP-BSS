...
function viewAdminHome(){
  ...
  return `${title('Operativni dashboard','Petak, 10. srpnja 2026. · pregled firme u stvarnom vremenu.',`${pill(`Demo ${APP_VERSION} · ${APP_STAGE}`)} ${pill(state.terminal.online?'Online':'Offline')}`)}
    <section class="card hero dashboard-hero"><div><div class="eyebrow">Dnevni sažetak</div><h2>${metrics.present} od ${metrics.active} radnika trenutačno je evidentirano</h2><p>Ključna odstupanja, odsutnosti i otvorene odluke prikazane su u četiri pokazatelja ispod.</p></div></section>
    ...
    <div class="dashboard-layout"><div class="dashboard-primary">
      <section class="card table-card"><div class="table-card-heading"><div><h2>Dnevni pregled evidencije</h2><p>Prijave, odjave i otvorene smjene po radnom danu.</p></div><button class="link-btn" data-bss-action="navigate('attendance')">Otvori evidenciju →</button></div>${weeklyAttendanceTable(weekly)}</section>
...
function viewWorkerHome(){
  ...
  return `${title(`Pozdrav, ${worker.name.split(' ')[0]}`,'Ovdje su isključivo tvoji podaci.',pill(worker.status))}
    ...
    <div class="stats-grid worker-home-summary"><div class="stat"><div class="num">${vacationRemaining(worker.id)}</div><div class="lab">Preostali dani godišnjeg</div><div class="trend">Detalji su u Moj godišnji</div></div><div class="stat"><div class="num">${ownRequests.filter(request=>request.status==='Na čekanju').length}</div><div class="lab">Otvoreni zahtjevi</div><div class="trend">Statusi su u Mojim zahtjevima</div></div></div>
    <div class="notice info">Mjesečni sati, zapisi i korekcije nalaze se samo u modulu <b>Moji sati</b>.</div>`;
}
function viewManagerHome(){
  ...
  return `${title('Dashboard mojeg tima',`Odjeli: ${role().departments.join(' i ')} · 10. srpnja 2026.`,pill(`${team.length} radnika`))}
    ...
    <div class="dashboard-layout"><div class="dashboard-primary"><section class="card table-card"><div class="table-card-heading"><div><h2>Dnevni pregled tima</h2><p>Prijave, odjave i otvorene smjene po radnom danu.</p></div><button class="link-btn" data-bss-action="navigate('attendance')">Evidencija tima →</button></div>${weeklyAttendanceTable(weekly)}</section><section class="card table-card"><div class="table-card-heading"><div><h2>Radnici mojeg tima</h2><p>Status, odjel i dodijeljena smjena.</p></div></div>${workerTable(team,false)}</section></div><aside class="dashboard-secondary"><section class="card"><div class="card-heading"><div><h2>Upozorenja i odluke</h2><p>Samo stavke iz dodijeljenih odjela.</p></div><span class="alert-total">${alerts.length}</span></div><div class="alert-list">${alerts.map(alert=>`<button class="alert-item ${alert.tone}" data-bss-action="${alert.action||`navigate('${alert.target}')`}"><span>${alert.icon}</span><div><b>${escapeHtml(alert.title)}</b><small>${escapeHtml(alert.text)}</small></div><i>›</i></button>`).join('')||'<div class="empty-state compact">Nema otvorenih upozorenja.</div>'}</div></section></aside></div>`;
}
...
function recordTable(records,heading = 'Evidencija',showDepartment = false,showDetails = false,showWorker = true){
...
function viewMyTime(){
  ...
  return `${title('Moja evidencija',`Samo tvoji zapisi radnog vremena za ${worker.name}.`,pill(monthDisplay(myTimeMonth)))}
    ...
    ${recordTable(records,'Moji zapisi',false,true,false)}
    ${correctionForm()}`;
}
...
function viewWorkers(){
  ...
  return `${title(isAdmin?'Radnici':'Moj tim',isAdmin?'Upravljanje zaposlenicima, smjenama i RFID karticama.':'Prikaz je ograničen na odjele voditelja.',isAdmin?'<button class="btn" data-bss-action="openWorkerModal()">Dodaj radnika</button>':'')}
    ...
    <div class="card table-card"><div class="table-card-heading"><div><h2>${isAdmin?'Popis radnika':'Radnici u mojem opsegu'}</h2><p>Profil, dodijeljena smjena, status i administrativni detalji.</p></div>${pill(`${workers.length} radnika`)}</div>${workerTable(workers,isAdmin)}</div>`;
}
...
function viewShifts(){
  ...
  return `${title('Smjene i pravila',isAdmin?'Dodavanje i uređivanje smjena te broj dodijeljenih radnika.':'Smjene su dostupne samo za čitanje.',isAdmin?'<button class="btn" data-bss-action="openShiftModal()">Dodaj smjenu</button>':'')}
    ...
    }).join('')}</tbody></table></div></section><div class="notice info">Smjena određuje planirano vrijeme, pauzu i toleranciju. Obračun dodataka za noćni i prekovremeni rad nije dio ovog statičnog demo-prikaza.</div>`;
}
...
function requestTable(requests,isApprover=false){
  const body=requests.map(request=>{
    ...
    return `<tr class="leave-request-row" data-request-id="${request.id}" data-status="${escapeHtml(request.status)}"><td><b>${escapeHtml(worker?.name||'Nepoznat radnik')}</b><br><span class="small-muted">${escapeHtml(worker?.dept||'—')} · poslano ${escapeHtml(request.submittedAt||'nije evidentirano')}</span></td><td><b>${escapeHtml(request.type)}</b><br><span class="small-muted">${escapeHtml(request.note||'Bez dodatne napomene.')}</span></td><td>${escapeHtml(rangeLabel(request.start,request.end))}</td><td>${days}</td><td class="${conflicts.length?'negative':''}">${conflicts.length?`<b>${conflictText}</b>`:conflictText}</td><td>${pill(request.status)}</td><td>${decision}</td><td>${approverAction||workerAction||'—'}</td></tr>`;
  }).join('');
  return `<div class="table-wrap"><table class="compact-table requests-table"><thead><tr><th>Radnik</th><th>Vrsta i napomena</th><th>Razdoblje</th><th>Radni dani</th><th>Preklapanje</th><th>Status</th><th>Odluka</th><th>Radnja</th></tr></thead><tbody>${body||'<tr><td colspan="8"><div class="empty-state">Nema zahtjeva za odabrane kriterije.</div></td></tr>'}</tbody></table></div>`;
}
...
function viewRequests(){
  ...
  const summaryHeaders=isWorker?['Fond 2026.','Odobreno','Rezervirano','Dostupno']:['Čeka odluku','S preklapanjem','Odobreni radni dani','Zatvoreno bez odsutnosti'];
  const summaryValues=isWorker?[balance.allowance,balance.used,balance.reserved,balance.available]:[counts['Na čekanju'],conflicts,approvedDays,counts.Odbijeno+counts.Poništeno];
  const summary=`<section class="card table-card request-summary-card"><div class="table-card-heading"><div><h2>Sažetak zahtjeva</h2><p>Brojčani pregled trenutačnog opsega i statusa.</p></div></div><div class="table-wrap"><table class="compact-table summary-table"><thead><tr>${summaryHeaders.map(label=>`<th>${escapeHtml(label)}</th>`).join('')}</tr></thead><tbody><tr>${summaryValues.map(value=>`<td><b>${value}</b></td>`).join('')}</tr></tbody></table></div></section>`;
  return `${title(isWorker?'Moji zahtjevi':currentRole==='manager'?'Zahtjevi mojeg tima':'Zahtjevi za odsutnost',isWorker?'Tvoji zahtjevi, odluke i raspoloživi dani.':'Odluke s napomenom, provjerom preklapanja i audit tragom.',pill(`${counts['Na čekanju']} na čekanju`))}<div class="card request-control-card">${requestTabs(scoped)}<div class="request-search"><input id="requestSearch" aria-label="Traži zahtjeve" placeholder="Ime, odjel, vrsta ili napomena" value="${escapeHtml(requestSearch)}"><button class="btn" data-bss-action="applyRequestSearch()">Traži</button><button class="btn secondary" data-bss-action="clearRequestFilters()">Očisti</button></div></div>${summary}${isApprover?'<div class="notice">Preklapanje je upozorenje za kapacitet odjela, ne automatska zabrana. Odluka i napomena ostaju u audit tragu.</div>':''}<section class="card table-card"><div class="table-card-heading"><div><h2>Tablični pregled zahtjeva</h2><p>Filtrirani zahtjevi, razdoblja, statusi i odluke.</p></div>${pill(`${requests.length} zahtjeva`)}</div>${requestTable(requests,isApprover)}</section>${isWorker?vacationRequestForm():''}<button class="btn secondary block" data-bss-action="navigate('vacations')">Otvori kalendar godišnjih</button>`;
}
...
function reportPreview(data){
  return `<div class="card table-card report-preview"><div class="table-card-heading"><div><h2>Tablični pregled · ${escapeHtml(data.title)}</h2><p>${escapeHtml(data.period)} · ${escapeHtml(data.scope)} · izvoz sadrži isti skup redaka.</p></div>${pill(`${data.rows.length} redaka`)}</div><div class="table-wrap"><table class="report-table"><thead><tr>${data.headers.map(header=>`<th>${escapeHtml(header)}</th>`).join('')}</tr></thead><tbody>${data.rows.map(values=>`<tr>${values.map(value=>`<td>${escapeHtml(value)}</td>`).join('')}</tr>`).join('')||`<tr><td colspan="${data.headers.length}"><div class="empty-state">Nema podataka za odabrane kriterije.</div></td></tr>`}</tbody></table></div><div class="table-summary"><span>${data.rows.length} redaka · ${escapeHtml(data.period)}</span><span>XLSX je poslovni, a CSV tehnički izvoz</span></div></div>`;
}
...
function viewReports(){
  ...
  return `${title('Izvještaji','Kontrolirani podaci za provjeru i predaju knjigovodstvu.',pill(currentRole==='accountant'?'Samo čitanje':`${data.rows.length} redaka`))}
    ...
    <section class="card report-filter-card"><div class="card-heading"><div><h2>Kriteriji izvještaja</h2><p>Filtri određuju tablični pregled te poslovni XLSX i tehnički CSV izvoz.</p></div></div><div class="report-filter-bar"><label><span>Mjesec</span><input id="reportMonth" type="month" min="2026-01" max="2026-12" value="${reportFilters.month}"></label><label><span>Odjel</span><select id="reportDept" data-bss-change="updateReportDepartment(this.value)">${departmentOptions(reportFilters.department)}</select></label><label><span>Radnik</span><select id="reportWorker"><option value="Svi" ${reportFilters.workerId==='Svi'?'selected':''}>Svi radnici</option>${scopedWorkers.map(worker=>`<option value="${worker.id}" ${String(worker.id)===String(reportFilters.workerId)?'selected':''}>${escapeHtml(worker.name)}</option>`).join('')}</select></label><button class="btn" data-bss-action="applyReportFilters()">Primijeni filtre</button></div></section>
    ${reportPreview(data)}
    <div class="report-layout"><section class="card report-export"><div class="card-heading"><div><h2>Izvoz izvještaja</h2><p>XLSX je glavni poslovni format; CSV ostaje tehnički format za razmjenu podataka.</p></div></div><div class="export-file primary-export"><span>XLSX · Excel · poslovni izvoz</span><b>${escapeHtml(data.filenameBase)}.xlsx</b></div><div class="export-file"><span>CSV · UTF-8 · tehnički izvoz</span><b>${escapeHtml(data.filenameBase)}.csv</b></div><div class="btns"><button class="btn" data-bss-action="downloadReport('xlsx')" ${data.rows.length?'':'disabled'}>Preuzmi XLSX</button><button class="btn secondary" data-bss-action="downloadReport('csv')" ${data.rows.length?'':'disabled'}>Tehnički CSV</button></div><div class="last-report"><span>Posljednja radnja</span><b>${escapeHtml(state.lastReport)}</b></div></section><section class="card report-quality"><div class="card-heading"><div><h2>Kontrola podataka</h2><p>Stavke koje treba razumjeti prije izvoza.</p></div></div>${data.quality.map((item,index)=>`<div class="quality-item"><i>${index+1}</i><span>${escapeHtml(item)}</span></div>`).join('')}</section></div>
    ${reportHistoryView()}`;
}

...
| R-002 | Valuable work remains stacked on obsolete pre-Phase-0 branches. | High | High | MITIGATING | Do not merge PRs #28 or #31 wholesale; compare against current `main`, extract unique useful work into focused PRs and then supersede old drafts. | No active long-lived PR depends on the obsolete PR #27 base and retained capabilities are accounted for. |
...
| R-012 | Two large backend service modules create maintainability and review risk. | High | Medium | ACCEPTED / MITIGATING | Freeze uncontrolled growth through a recalibrated architecture budget, then decompose incrementally with tests. | Architecture budget is active on current `main` and module scope is reduced without contract regression. |
...
| R-017 | Root `AGENTS.md` still reflects pre-Phase-0 sources of truth and ambiguously treats Cloudflare as the deployment platform for the whole system. | High | High | MITIGATING | Complete post-merge BSS OS truth sync, then replace with `AGENTS.md v2` that names current sources of truth and separates Cloudflare frontend/Preview hosting from the Node/Fastify backend runtime decision. | `AGENTS.md v2` merged from current `main`, reviewed against BSS OS and infrastructure ADR with green governance checks. |
